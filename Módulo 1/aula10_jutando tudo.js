...
function verificarAposentadoria(nomeColaborador){
  ...
  for (index = 0; index < colaboradores.length; index++){
    if (nomeColaborador == colaboradores[index]){
      colaborador = true
      idade = idades[index]
      tempoTrabalhado = temposDeTrabalho[index]
    }
  }

if (colaborador){
  porIdade = (idade >= 65)
  porTempoContribuicao = (tempoTrabalhado >= 30)
  porIdadeETempoContribuicao = (idade >= 60 && tempoTrabalhado >= 25)

  if (porIdade || porTempoContribuicao || porIdadeETempoContribuicao){
